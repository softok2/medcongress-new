...
from MedCongressAdmin.forms.congres_forms import OrganizadorForm,UsuarioForms
from MedCongressAdmin.apps import validarUser
from django.db.models import Q
import json   

class OrganizadorListView(validarUser,ListView):
    model = Organizador
    context_object_name = 'organizadores'
    template_name = 'MedCongressAdmin/organizador/listar.html'

    def get_context_data(self, **kwargs):
        context=super(OrganizadorListView,self).get_context_data(**kwargs)
        context['search']=self.request.GET.get('search')
        return context

class  OrganizadorCreateView(validarUser,CreateView):
    form_class = OrganizadorForm
    success_url = reverse_lazy('MedCongressAdmin:Organizadores_list')
    template_name = 'MedCongressAdmin/organizador/form.html'
    def get_success_url(self):
        url= reverse_lazy('MedCongressAdmin:Organizadores_list')
    ...
    def form_valid(self, form):
        ...
        return super(OrganizadorCreateView, self).form_valid(form)

    def get_context_data(self, **kwargs):
        context = super(OrganizadorCreateView, self).get_context_data(**kwargs)
        context['search']=self.request.GET.get('search')
        organizador=Organizador.objects.all()
        return context
class OrganizadorDeletedView(validarUser,DeleteView):
    model = Organizador
    success_url = reverse_lazy('MedCongressAdmin:Organizadores_list')

    

class vTableAsJSONOrganizadores(TemplateView):
    template_name = 'MedCongressAdmin/asig_congress_form.html'
    
    def get(self, request, *args, **kwargs):
        #arreglo con las columnas de la BD a filtrar
        col_name_map = ['user__usuario__first_name','user__usuario__email','congreso__titulo']
           
        # #listado que muestra en dependencia de donde estes parado
        object_list = Organizador.objects.all()
        ...
        if len(search_text) > 0:
            filtered_object_list = object_list.filter(Q(user__usuario__last_name__icontains=search_text) | Q(user__usuario__email__icontains=search_text)|Q(user__usuario__first_name__icontains=search_text)|Q(congreso__titulo__icontains=search_text))
